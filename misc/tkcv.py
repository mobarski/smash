...
class HIST:
	...
	def set_dict(self,t,k,d,ts):
		items = [(t,k,c,self.serialize(v),ts) for c,v in d.items()]
		self.cursor.executemany('insert into tkcv_hist values (?,?,?,?,?)',items)
class NOHIST:
	def __init__(self): pass
	def set(self,t,k,c,v,ts): pass
	def set_dict(self,t,k,d,ts): pass


class TKCV(S,LINK,NOHIST):
	...
	def get_dict(self,t,k,cols=None,default=None):
		"Get column->value dictionary for all columns or for selected columns"
		if cols==None:
...
if __name__=="__main__":
	...
	if 0:
		...
		print(db.get_dict('usr',1,'age xxx name'))
		db.truncate('usr')
		print(db.get_dict('usr',2,'name xxx age'))
