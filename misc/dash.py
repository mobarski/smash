...
def parse(text,cnt=0,default=None,comment='#<>!@+-;*',empty='.'):
	...
	lines = [x.strip() for x in re.split('\n\r|\r\n|\n|\r',text.strip()) if x.strip() and x.strip()[0] not in comment]		
...
# SCITE:  file.patterns.diff=$(file.patterns.diff);*.dash
# TODO: SCITE tab.width in .dash file
